...
npm test                # vitest — server route tests (needs supabase running)
                        # + the pure client unit tests (friends/pacts
                        # normalization, scheduler engine)
...
rows never carry the `'u-me'` pre-auth placeholder), the friends graph and
pacts are server-side (solo pacts: create/list/cancel + interim
complete/settle, keeper read access; mutual creates are rejected until
Proposals land — ADR-0006/0007/0008), and `EXPO_PUBLIC_API_URL` is required
— put it in `.env` (copy `.env.example`). Booting without it fails fast at
module load; there is no offline or demo fallback. Check-ins and
...
- **Scheduler engine** (`src/lib/engine.ts`): on launch (signed in +
  ...
  days and emits breach notifications — for pacts the signed-in user
  *created* only (a kept pact's check-ins live on the creator's device;
  nothing is fabricated from their absence). Durable status transitions
  (goal completion, end-of-term settlement) are not flipped locally: the
  store pushes them through the interim `/pacts/:id/complete` and
  `/pacts/:id/settle` endpoints, then refreshes. It mirrors the future
  backend cron jobs — keep its rules in sync with the spec in README.
